    ...
    <div className="space-y-8 md:space-y-12 pb-20">
          ...
          >
            {notices.map((notice, idx) => (
              ...
              >
                ...
                <div className={cn(
                  "p-3 md:p-5 rounded-2xl relative z-10 shadow-2xl border border-white/5",
                  ...
                  <Megaphone size={20} className="md:w-7 md:h-7" />
                </div>
                <div className="flex-1 min-w-0 relative z-10">
                  <div className="flex justify-between items-center mb-1 md:mb-3">
                    <h4 className="font-black text-white text-sm md:text-lg truncate uppercase group-hover:text-indigo-400 transition-colors">{notice.title}</h4>
                    <span className="text-[10px] font-black text-slate-500 uppercase shrink-0 ml-2 bg-white/5 px-2 py-0.5 rounded-full border border-white/5">
                      {format(new Date(notice.createdAt), 'hh:mm a')}
                    </span>
                  </div>
                  <p className="text-[11px] md:text-sm text-slate-400 leading-relaxed line-clamp-2 font-bold group-hover:text-slate-300 transition-colors">{notice.content}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      >
        {[
          ...
          <motion.div 
            ...
            className="glass-card p-6 md:p-12 border border-white/5 relative overflow-hidden group shadow-2xl"
          >
            <div className={cn(
            ...
            <div className="flex flex-col gap-4 md:gap-8 relative z-10">
              <div className={cn(
                "w-12 h-12 md:w-20 md:h-20 rounded-2xl md:rounded-3xl flex items-center justify-center shadow-2xl border border-white/10 group-hover:rotate-12 transition-transform",
              ...
              <div>
                <p className="text-slate-500 text-[10px] md:text-xs font-black uppercase mb-1 md:mb-3 opacity-60">{stat.label}</p>
                <h3 className={cn("text-2xl md:text-5xl font-black text-white")}>
                  ৳{stat.value.toLocaleString()}
                </h3>
              </div>
            </div>
            <div className="absolute bottom-0 left-0 w-full h-0.5 md:h-1 bg-gradient-to-r opacity-30 group-hover:opacity-100 transition-opacity" style={{ backgroundImage: `linear-gradient(to right, var(--tw-gradient-from), var(--tw-gradient-to))` }} />
          </motion.div>
        ))}
      </motion.div>
        ...
        {[
          ...
          >
            <Link to={action.to} className={cn(
              ...
              <div className="p-3 md:p-5 bg-white/10 rounded-2xl group-hover:rotate-12 group-hover:scale-110 transition-all relative z-10 shadow-2xl border border-white/20 backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-12">
        <motion.div 
          ...
          className="lg:col-span-2 glass-card p-6 md:p-12 border border-white/5 shadow-[0_40px_80px_rgba(0,0,0,0.3)] relative overflow-hidden"
        >
          ...
          <div className="flex items-center justify-between mb-8 md:mb-12 relative z-10">
            <div className="flex items-center gap-4 md:gap-6">
              ...
              <div>
                <h2 className="text-xl md:text-3xl font-black text-white uppercase">আয়-ব্যয় গ্রাফ</h2>
                <p className="text-[10px] md:text-xs font-black text-slate-500 uppercase mt-1">বিগত ৬ মাসের আর্থিক চিত্র</p>
              </div>
            </div>
          </div>
          <div className="h-[250px] md:h-[400px] relative z-10">
            {chartData ? <Line data={chartData} options={chartOptions} /> : <div className="h-full bg-white/5 animate-pulse rounded-[2rem]" />}
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          className="glass-card p-6 md:p-12 border border-white/5 flex flex-col shadow-[0_40px_80px_rgba(0,0,0,0.3)] relative overflow-hidden"
        >
          ...
          <div className="flex items-center gap-4 md:gap-6 mb-8 md:mb-12 relative z-10">
            <div className="p-3 md:p-4 bg-purple-500/10 rounded-2xl text-purple-400 border border-purple-500/20 shadow-inner">
              <PieChartIcon size={24} className="md:w-8 md:h-8" />
            </div>
            <div>
              <h2 className="text-xl md:text-3xl font-black text-white uppercase">ব্যয় বিভাজন</h2>
              <p className="text-[10px] md:text-xs font-black text-slate-500 uppercase mt-1">খাত ভিত্তিক খরচ</p>
            </div>
          </div>
          <div className="flex-1 flex items-center justify-center min-h-[250px] md:min-h-[350px] relative z-10">
            {pieData ? (
              <Pie 
                data={pieData} 
                options={{ 
              ...
              <div className="w-40 h-40 md:w-56 md:h-56 rounded-full border-[12px] md:border-[18px] border-white/5 border-t-indigo-500 animate-spin shadow-[0_0_50px_rgba(79,70,229,0.3)]" />
            )}
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12">
        ...
        >
          <div className="p-6 md:p-12 border-b border-white/5 flex justify-between items-center bg-white/5">
            <div className="flex items-center gap-4 md:gap-6">
              <div className="p-3 md:p-4 bg-emerald-500/10 rounded-2xl text-emerald-400 border border-emerald-500/20 shadow-inner">
                <HistoryIcon size={24} className="md:w-8 md:h-8" />
              </div>
              <div>
                <h2 className="text-xl md:text-3xl font-black text-white uppercase">সাম্প্রতিক লেনদেন</h2>
          ...
          <div className="divide-y divide-white/5">
            {recentTransactions.length === 0 ? (
              <div className="p-12 md:p-24 text-center">
                <p className="text-slate-500 font-black uppercase opacity-40">কোন লেনদেন পাওয়া যায়নি।</p>
              </div>
            ) : (
              recentTransactions.map((t, i) => (
                <motion.div 
                  ...
                  className="p-5 md:p-10 flex items-center justify-between hover:bg-white/5 transition-all group cursor-pointer relative overflow-hidden"
                >
                  ...
                  <div className="flex items-center gap-4 md:gap-8 relative z-10">
                    <div className={cn(
                      ...
                      {t.type === 'income' ? <TrendingUp size={20} className="md:w-7 md:h-7" /> : <TrendingDown size={20} className="md:w-7 md:h-7" />}
                    </div>
                    <div>
                      <p className="font-black text-white text-sm md:text-xl uppercase group-hover:text-indigo-400 transition-colors">{t.userName}</p>
                      <div className="flex items-center gap-2 md:gap-4 mt-1 md:mt-2">
                        <span className="text-[9px] md:text-xs text-slate-500 font-black uppercase bg-white/5 px-2 py-0.5 rounded-full">{t.category}</span>
                        <span className="text-[9px] md:text-slate-600 font-black uppercase">{format(new Date(t.date), 'dd MMM yyyy')}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right relative z-10">
                    ...
                    <span className={cn(
                      "text-[8px] md:text-xs px-2 py-1 md:px-4 md:py-1.5 rounded-full font-black uppercase mt-1 md:mt-3 inline-block border shadow-sm",
        ...
        >
          <div className="p-6 md:p-12 border-b border-white/5 flex justify-between items-center bg-white/5">
            <div className="flex items-center gap-4 md:gap-6">
              <div className="p-3 md:p-4 bg-amber-500/10 rounded-2xl text-amber-400 border border-amber-500/20 shadow-inner">
                <Users size={24} className="md:w-8 md:h-8" />
              </div>
              <div>
                <h2 className="text-xl md:text-3xl font-black text-white uppercase">শীর্ষ দাতা</h2>
                <p className="text-[10px] md:text-xs font-black text-slate-500 uppercase mt-1">সর্বোচ্চ অবদানকারী সদস্যগণ</p>
              </div>
            </div>
            <div className="hidden md:flex items-center gap-3 px-4 py-2 bg-indigo-500/10 rounded-2xl border border-indigo-500/20 text-indigo-400 text-xs font-black uppercase">
          ...
          <div className="divide-y divide-white/5">
            {users.map((u, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-4 md:gap-8 relative z-10">
                  <div className={cn(
                    "w-10 h-10 md:w-16 md:h-16 rounded-xl md:rounded-[1.5rem] flex items-center justify-center font-black text-sm md:text-xl transition-all border shadow-2xl",
                  ...
                  <div>
                    <p className="font-black text-white text-sm md:text-xl uppercase flex items-center gap-2 md:gap-3 group-hover:text-amber-400 transition-colors">
                      {u.name} {u.role === 'admin' && <Shield size={14} className="md:w-4.5 md:h-4.5 text-indigo-400" />}
                    </p>
                    <p className="text-[9px] md:text-xs text-slate-500 font-black uppercase mt-1 md:mt-2 flex items-center gap-1 md:gap-2">
                ...
                <div className="text-right relative z-10">
                  <p className="text-lg md:text-3xl font-black text-white group-hover:text-amber-400 transition-colors">
                    {u.contributionVisibility ? `৳${u.totalContribution.toLocaleString()}` : '৳••••'}
                  </p>
                  <p className="text-[9px] md:text-xs font-black text-slate-600 uppercase mt-1">মোট অবদান</p>
                </div>
              </motion.div>
            ))}
          </div>
